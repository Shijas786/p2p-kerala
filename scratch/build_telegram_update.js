...
.phone-shell { background: #0f0f11; border-radius: 40px; padding: 12px; box-shadow: inset 0 0 0 2px #3a3a3c, inset 0 0 0 6px #121212; position: relative; }
.phone-screen { background: var(--ma-bg); border-radius: 30px; overflow: hidden; position: relative; display: flex; flex-direction: column; height: 600px; }
...
:root { --ma-bg: #000000; --ma-surface: #1e1e1e; --ma-border: #333333; --ma-green: #4caf50; --ma-red: #ff4d4d; --ma-text: #ffffff; --ma-sub: #a0a0a0; }
.ma-content { flex: 1; overflow-y: auto; padding: 34px 0 60px 0; }
.ma-nav { height: 60px; background: rgba(30,30,30,0.95); backdrop-filter: blur(10px); border-top: 1px solid var(--ma-border); display: flex; justify-content: space-around; align-items: center; position: absolute; bottom: 0; width: 100%; z-index: 10; }
...
.ma-modal-content { background: var(--ma-surface); border-radius: 24px 24px 0 0; padding: 2rem 1.5rem; transform: translateY(100%); animation: slidePopup 0.4s cubic-bezier(0.16, 1, 0.3, 1) forwards; }

/* Wallet Connect popup styles */
.wc-title { display: flex; justify-content: space-between; align-items: center; font-size: 1.2rem; font-weight: 700; color: #fff; margin-bottom: 1.5rem; }
.wc-close { width: 32px; height: 32px; border-radius: 50%; background: rgba(255,255,255,0.1); display: flex; align-items: center; justify-content: center; font-size: 0.8rem; color: #fff; }
.wc-opt { display: flex; align-items: center; gap: 1rem; padding: 1rem; background: rgba(255,255,255,0.05); border-radius: 16px; margin-bottom: 0.8rem; transition: background 0.2s;}
.wc-opt:hover { background: rgba(255,255,255,0.1); }
.wc-ico { width: 40px; height: 40px; border-radius: 10px; background: #222; }
.wc-name { font-size: 1rem; font-weight: 600; color: #fff; flex:1;}
.wc-tag { background: rgba(76, 175, 80, 0.2); color: #4caf50; padding: 0.2rem 0.5rem; border-radius: 6px; font-size: 0.6rem; font-weight: 700;}

/* Telegram Screen */
.tg-bg { background: #000; position: absolute; inset: 0; z-index: 50; display: flex; flex-direction: column; opacity:0; animation: fadeOverlay 0.3s forwards;}
...
.tg-msg { background: #182533; border-radius: 12px 12px 12px 0; width: 92%; box-shadow: 0 1px 2px rgba(0,0,0,0.2); opacity: 0; transform: scale(0.95); transform-origin: left bottom; animation: popMsg 0.4s 0.3s forwards cubic-bezier(0.17, 0.89, 0.32, 1.28); display:flex; flex-direction:column;}
...
function playFlow(flow) {
  ...
  // Change title in menu based on flow
  if (flow === 'vault') {
  ...
  if (flow === 'buy' || flow === 'sell') {
    ...
    demoTimeouts.push(setTimeout(() => {
      screen.innerHTML = notch + \`
        ...
        <div class="ma-content">
          ...
          <div class="ma-input"><div class="ma-input-val" id="type-amt"></div><div class="ma-input-suffix">USDT</div></div>
      ...
      demoTimeouts.push(setTimeout(() => { amt.innerText = "2"; }, 1500));
      demoTimeouts.push(setTimeout(() => { amt.innerText = "25"; }, 1650));
      demoTimeouts.push(setTimeout(() => { amt.innerText = "25.08"; amt.classList.remove('typing'); }, 1850));
      demoTimeouts.push(setTimeout(() => { c2.style.transform = 'translate(150px, 450px)'; }, 2350));
      demoTimeouts.push(setTimeout(() => { pub.classList.add('btn-clicked'); }, 3050));
      demoTimeouts.push(setTimeout(() => { c2.style.opacity = '0'; }, 3150));

      // Screen 3: Telegram Broadcast matching screenshot EXACTLY
      demoTimeouts.push(setTimeout(() => { 
        tgOverlay.innerHTML = \`
          <div class="tg-bg">
            <div class="tg-header">
              ...
              <div class="tg-msg">
                <div class="tg-msg-inner">
                  ...
                  <div class="tg-msg-text">
                    📢 <b>New \${isBuy ? 'BUY' : 'SELL'} Ad!</b><br/><br/>
                    \${isBuy ? '🟢 @Medphysicist wants to buy' : '🔴 @pcoke_og wants to sell'} <b>25.08 USDT</b><br/>
                    💰 Rate: ₹\${isBuy ? '98' : '98.5'}/USDT<br/>
                    🧾 Total: ₹\${isBuy ? '2,458' : '2,470'}<br/>
                    🔗 Chain: BSC<br/>
                    💳 Payment: DIGITAL_RUPEE
                  </div>
                  <div class="tg-msg-time">05:23 PM</div>
                </div>
                <div class="tg-inline-btn">⚡ \${isBuy ? 'Sell Now' : 'Buy Now'} ↗</div>
              </div>
            </div>
          </div>
        \`;
    ...
    screen.innerHTML = notch + \`
      ...
      <div class="ma-content">
        <div class="aw-hero">
          ...
          <div class="aw-actions"><div class="aw-dep" id="btn-dep">Deposit</div><div class="aw-send">Send</div></div>
        </div>
      ...
      <div class="ma-modal" id="modal" style="display:none; opacity:0;">
        <div class="ma-modal-content">
          <div class="wc-title"><span>Connect Wallet</span><div class="wc-close">✕</div></div>
          <div class="wc-opt"><div class="wc-ico" style="background: url('https://reown.com/favicon.ico') center/cover; background-color:#222;"></div><div class="wc-name">AppKit Mobile</div><div class="wc-tag">RECOMMENDED</div></div>
          <div class="wc-opt"><div class="wc-ico" style="background: url('https://upload.wikimedia.org/wikipedia/commons/3/36/MetaMask_Fox.svg') center/cover; background-color:#fff;"></div><div class="wc-name">MetaMask</div></div>
          <div class="wc-opt" style="margin-bottom:0;"><div class="wc-ico" style="background: url('https://trustwallet.com/assets/images/favicon.png') center/cover; background-color:#fff;"></div><div class="wc-name">Trust Wallet</div></div>
        </div>
      </div>
    \`;
    ...
    demoTimeouts.push(setTimeout(() => { c.style.opacity = '1'; c.style.transform = 'translate(60px, 140px)'; }, 500));
    demoTimeouts.push(setTimeout(() => { dep.classList.add('btn-clicked'); }, 1200));
    ...
    demoTimeouts.push(setTimeout(() => { c.style.transform = 'translate(150px, 450px) scale(0.9)'; }, 2400));
    demoTimeouts.push(setTimeout(() => { c.style.opacity = '0'; }, 2600));
  }
}
...
console.log("Ultra Realistic UI Built.");
